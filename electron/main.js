...
ipcMain.handle('auth:login', (event, { tenantCode, username, password }) => {
  return login(tenantCode, username, password);
});

ipcMain.handle('auth:roles', () => ROLES);

ipcMain.handle('auth:changePassword', (event, { userId, newPassword }) => {
  return requirePasswordChangeComplete(userId, newPassword);
});

ipcMain.handle('tenant:current', (event, { userId }) => {
...
// ===================== USER MANAGEMENT =====================
ipcMain.handle('users:list', (event, { requestingUserId }) => {
  const user = requirePermission(requestingUserId, 'canManageUsers');
  return db.prepare(`
...
ipcMain.handle('users:create', (event, { data, requestingUserId }) => {
  const user = requirePermission(requestingUserId, 'canManageUsers');
  ...
  if (branchId) requireBranchAccess(requestingUserId, branchId);
  ...
  logAudit({
    ...
    userId: requestingUserId, tenantId: user.tenant_id,
  });
  return { id };
});

ipcMain.handle('users:setActive', (event, { userId, active, requestingUserId }) => {
  const admin = requirePermission(requestingUserId, 'canManageUsers');
  const { row } = requireTenantRecord(requestingUserId, 'users', userId);
  if (userId === requestingUserId) throw new Error('You cannot disable your own account');
  db.prepare('UPDATE users SET active=?, updated_at=datetime(\'now\') WHERE id=? AND tenant_id=?')
    .run(active ? 1 : 0, userId, admin.tenant_id);
  logAudit({ table: 'users', recordId: userId, action: 'update', oldValue: { active: row.active }, newValue: { active: !!active }, userId: requestingUserId, tenantId: admin.tenant_id });
  return { success: true };
});

ipcMain.handle('users:resetPassword', (event, { userId, temporaryPassword, requestingUserId }) => {
  const admin = requirePermission(requestingUserId, 'canManageUsers');
  ...
  logAudit({ table: 'users', recordId: userId, action: 'password_reset', oldValue: { must_change_password: row.must_change_password }, newValue: { must_change_password: 1 }, userId: requestingUserId, tenantId: admin.tenant_id });
  return { success: true };
});

// ===================== BRANCHES =====================
ipcMain.handle('branches:list', (event, { userId }) => {
  const user = getAuthenticatedUser(userId);
...
// ===================== ITEM MASTER =====================
ipcMain.handle('items:list', (event, { userId }) => {
  const user = requireModuleAccess(userId, 'item_master');
  if (user.role === 'super_admin' || user.role === 'auditor') {
...
ipcMain.handle('items:create', (event, { data, userId }) => {
  ...
  try {
    ...
    `).run({
      ...
      hsn_code: data.hsn_code || '7113', branch_id: branchId, created_by: userId,
    });
  } catch (err) {
  ...
  logAudit({ table: 'items', recordId: id, action: 'insert', newValue: data, userId, tenantId: user.tenant_id });
  return { id };
});

ipcMain.handle('items:update', (event, { id, data, userId }) => {
  const user = requireModuleAccess(userId, 'item_master');
  const { row: oldValue } = requireTenantRecord(userId, 'items', id);
  ...
  logAudit({ table: 'items', recordId: id, action: 'update', oldValue, newValue: data, userId, tenantId: user.tenant_id });
  return { success: true };
});

// ===================== CUSTOMERS =====================
ipcMain.handle('customers:list', (event, { userId }) => {
  const user = requireModuleAccess(userId, 'customers');
  return db.prepare('SELECT * FROM customers WHERE tenant_id=? ORDER BY created_at DESC').all(user.tenant_id);
});

ipcMain.handle('customers:search', (event, { userId, query }) => {
  const user = requireModuleAccess(userId, 'customers');
...
ipcMain.handle('customers:create', (event, { data, userId }) => {
  const user = requireModuleAccess(userId, 'customers');
  const id = newId();
  ...
  logAudit({ table: 'customers', recordId: id, action: 'insert', newValue: data, userId, tenantId: user.tenant_id });
  return { id };
});

// ===================== RATES =====================
ipcMain.handle('rates:latest', (event, { userId }) => {
  const user = requireModuleAccess(userId, 'rates');
  return db.prepare(`
...
ipcMain.handle('rates:set', (event, { metal, purity, rate_per_gram, userId }) => {
  const user = requirePermission(userId, 'canSetRates');
  if (user.role !== 'super_admin' && user.role !== 'auditor') requireBranchAccess(userId, user.branch_id);
  if (user.role === 'auditor') throw new Error('Auditor is read-only');
  ...
  db.prepare(`INSERT INTO rate_master (id, tenant_id, metal, purity, rate_per_gram, set_by) VALUES (?,?,?,?,?,?)`)
    .run(id, user.tenant_id, metal, purity, rate, userId);
  logAudit({ table: 'rate_master', recordId: id, action: 'insert', newValue: { metal, purity, rate_per_gram: rate }, userId, tenantId: user.tenant_id });
  return { id };
});

// ===================== BILLING =====================
ipcMain.handle('invoices:create', (event, { docType, customerId, branchId, lines, discount, oldGoldExchangeValue, convertedFromEstimateId, userId }) => {
  const user = requireModuleAccess(userId, 'billing');
  if (user.role === 'auditor') throw new Error('Auditor is read-only');
  const { branch } = requireBranchAccess(userId, branchId || user.branch_id);
  const effectiveBranchId = branch.id;
  const customer = customerId ? requireTenantRecord(userId, 'customers', customerId).row : null;
  ...
  const taxedLines = safeLines.map(line => {
    if (line.item_id) {
      const item = requireTenantRecord(userId, 'items', line.item_id).row;
  ...
  tx();
  logAudit({ table: 'invoices', recordId: invoiceId, action: 'insert', newValue: { docType, totals }, userId, tenantId: user.tenant_id });
...
ipcMain.handle('invoices:preview', (event, { customerId, branchId, lines, discount, oldGoldExchangeValue, userId }) => {
  const user = requireModuleAccess(userId, 'billing');
  const { branch } = requireBranchAccess(userId, branchId || user.branch_id);
  const customer = customerId ? requireTenantRecord(userId, 'customers', customerId).row : null;
  const interState = isInterState(branch.state_code, customer ? customer.gstin : null);
...
ipcMain.handle('invoices:getWithLines', (event, { invoiceId, userId }) => {
  const user = requireModuleAccess(userId, 'billing');
  const invoice = requireTenantRecord(userId, 'invoices', invoiceId).row;
...
ipcMain.handle('invoices:list', (event, { docType, userId }) => {
  const user = requireModuleAccess(userId, 'billing');
...
ipcMain.handle('export:table', (event, { table, format, userId }) => {
  const user = getAuthenticatedUser(userId);
...
ipcMain.handle('backup:manual', async (event, { destPath, userId }) => {
  const user = requirePermission(userId, 'canManageUsers');
  const result = await manualBackupTo(destPath);
  logAudit({ table: 'database', recordId: user.tenant_id, action: 'backup', newValue: { path: destPath, sha256: result.sha256 }, userId, tenantId: user.tenant_id });
  return result;
});
